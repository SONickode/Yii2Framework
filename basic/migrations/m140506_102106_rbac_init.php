...
class m140506_102106_rbac_init extends \yii\db\Migration
{
    ...
    public function up()
    {
        ...
        $viewAdminPanel->description = 'Access to admin panel';
        $auth->add($viewAdminPanel);

        $Login = $auth->createPermission('Login');
        $Login->description = 'User can log in';
        $auth->add($Login);
        ...
        $viewUser->description = 'View user info';
        $auth->add($viewUser);

        $updateUser = $auth->createPermission('updateUser');
        $updateUser->description = 'Update user info';
        $auth->add($updateUser);
        ...
        //roles
        $active = Yii::$app->authManager->createRole('active');
        $active->description = 'active user';
        $auth->add($active);
        $auth->addChild($active, $Login);
